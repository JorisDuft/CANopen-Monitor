        ...
        if ((updated || state_changed) && get_time_ms() - time_last_update > GUI_UPDATE_INTERVAL_MS)
        {
            ...
            for (int i = 1; i < 128; i++) 
            {
                if (node_list[i].visible) 
                {
                    ...
                    format_time(errtimebuf, sizeof(errtimebuf), node_list[i].time_lastError);
                    format_time(hbtimebuf, sizeof(hbtimebuf), node_list[i].time_lastHB);
