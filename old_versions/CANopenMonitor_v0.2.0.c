...
bool commObj_filter(int cob_id, thread_args_t* args)
{
    ...
    (cob_id >= 0x600 && cob_id <= 0x67F))
    {
        return (args->object_mask & OBJ_SDO);
    }

    // PDO
    if ((cob_id >= 0x180 && cob_id <= 0x57F))
    {
        return (args->object_mask & OBJ_PDO);
    }

    /* evt für später für Logging 
    if ((cob_id >= 0x000 && cob_id <= 0x07F) ||
        (cob_id >= 0x700 && cob_id <= 0x77F) ||
        (cob_id >= 0x080 && cob_id <= 0x0FF))
    {
 ...
 *      - sets the timeout for heartbeat messages

 * The configuration file should have the format:
 *      timeout_heartbeat_ms = 2000;
 */
void load_config(thread_args_t* gui_args)
{
    ...
    while (fgets(line, sizeof(line), file))
    {
        ...
        {
            if (strcmp(key, "timeout_heartbeat_ms") == 0)                   // Node timeout
            {
                long tmp = atol(value);
                if (tmp > 1)
                    gui_args->timeout_ms = (time_t)tmp;
            }
            else if (strcmp(key, "nodeID_filter") == 0)                     // node filter
            {
                parse_node_filter(gui_args, value);
            }
            else if (strcmp(key, "object_filter") == 0)                     // object filter
            {
...
void* thread_CANopenReader (void* arg) 
{
    ...
    while (1)                                                               // loop to read CAN messages and send to GUI thread
    {
        ...
        {
            msg.timestamp = get_time_ms();                                  // add timestamp to message
            int cob_id = frame.can_id & 0x7FF;
            msg.cob = cob_id >> 7;                                          // extract COB-ID (NodeID) from CAN ID
            msg.id = cob_id & 0x7F;                                         // extract NodeID from CAN ID
            msg.len = frame.can_dlc;
            for (int i = 0; i < msg.len; i++) {
                msg.data[i] = frame.data[i];
            }

            mq_send(mq, (const char*)&msg, sizeof(CANopenMessage), 0);      // send formatted message to GUI thread via message queue
        }
    }
    return NULL;
}
...
{    
    ...
    // extract configuration parameters from argument struct
    ...
    CANopenMessage msg;
    ssize_t bytes;
    bool updated = false;                                                   // flag to track if GUI needs to be updated
    bool state_changed = false;                                             // flag to track if node status changed (active/inactive)
    ...
    CDKSWINDOW *nodeWin = newCDKSwindow(cdkscreen, x, 1, heightNode, width, "NodeID | Status          | Error                               | LastError  | LastHB", 100, TRUE, FALSE);
    ...
    while (1) 
    {
        ...
        {    // read messages from queue until queue is empty
            ...
            else if (msg.cob == 0x1)
            {
                ...
                // 1. Sicherheitskopie für Node-Liste
                ...
                // 2. Anzeige mit Zeitstempel
                snprintf(em_msg, sizeof(em_msg),
                        "Node %d: %s | %02X %02X %02X %02X %02X %02X %02X %02X | %s",
                        msg.id,
                        error_desc,
            ...
            // SDO Error Messages (0x580 + NodeID)
            else if (msg.cob == 0xB)
            {
                if (msg.data[0] == 0x80)
                {
            ...
            {    
                if (!nodeID_filter(msg.id, gui_args))    { continue; }
                if (!commObj_filter(msg.cob, gui_args))  { continue; }
        ...
        if ((updated || state_changed) && get_time_ms() - time_last_update > GUI_UPDATE_INTERVAL_MS)      // update GUI every #define GUI_UPDATE_INTERVAL_MS xxx ms
        {
            ...
            for (int i = 1; i < 128; i++) 
            {
                ...
                {
                    if (get_time_ms() - node_list[i].time_lastHB > timeout_Heartbeat_ms)    // check if last Hartbeat is older than timeout
                    {
...
int main() 
{
    ...
    gui_args.timeout_ms = 1000; // default timeout for heartbeat messages in milliseconds
    gui_args.node_filter_enabled = false;
    gui_args.object_filter_enabled = false;
    gui_args.object_mask = OBJ_SDO | OBJ_PDO | OBJ_NMT;
    load_config(&gui_args); // load timeout and filter settings from config file, if available
    ...
    attr.mq_msgsize = sizeof(CANopenMessage);  // Puffergrösse für eine Nachricht
    attr.mq_curmsgs = 0;
